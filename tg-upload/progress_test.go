...
// state is a helper to build a ProgressState at a given percentage of total.
...
func TestProgressNextUnknownSize(t *testing.T) {
	...
	if _, ok := p.next(uploader.ProgressState{Total: -1, Uploaded: 5}); ok {
...
func TestProgressNextNonTTY(t *testing.T) {
	...
	if _, ok := p.next(state(0, 1000)); ok {
		t.Error("0%: expected no output")
	}
	line, ok := p.next(state(100, 1000)) // 10%
	if !ok {
	...
	if _, ok := p.next(state(150, 1000)); ok { // 15%, same bucket
		t.Error("15%: expected no output within the same 10% bucket")
	}
	if _, ok := p.next(state(200, 1000)); !ok { // 20%, next bucket
...
func TestProgressNextTTY(t *testing.T) {
	...
	line, ok := p.next(state(10, 1000)) // 1%
	if !ok {
	...
	if _, ok := p.next(state(10, 1000)); ok { // unchanged percent
		t.Error("expected no output when percent is unchanged")
	}
	final, ok := p.next(state(1000, 1000)) // 100%
